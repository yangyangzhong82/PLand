...
: impl(std::make_unique<Impl>(registry, priceService, selectionService)) {
    ...
    impl->mPlayerJoinListener = bus.emplaceListener<ll::event::PlayerJoinEvent>([this](ll::event::PlayerJoinEvent& ev) {
        auto& player = ev.self();
        if (player.isSimulatedPlayer()) return;
        if (!Config::cfg.land.leasing.enabled || !Config::cfg.land.leasing.notifications.loginTip) {
            return;
        }
        ...
        for (auto& land : lands) {
            ...
            if (remaining <= time_utils::toSeconds(Config::cfg.land.leasing.duration.renewalAdvance)) {
    ...
    impl->mPlayerEnterLandListener =
        bus.emplaceListener<event::PlayerEnterLandEvent>([this](event::PlayerEnterLandEvent& ev) {
            if (!Config::cfg.land.leasing.enabled || !Config::cfg.land.leasing.notifications.enterTip) {
                return;
            }
    ...
    for (auto& land : impl->mRegistry.getLands()) {
        if (land->isLeased()) {
            if (land->getLeaseState() == LeaseState::Active) {
                ...
                impl->mScheduler.push(
                    land->getLeaseEndAt() + time_utils::toSeconds(Config::cfg.land.leasing.freeze.duration),
                    land->getId()
                );
            }
        }
    ...
    ll::coro::keepThis([quit = impl->mQuit, sleep = impl->mSleep, this]() -> ll::coro::CoroTask<> {
        while (!quit->load()) {
            ...
            // 租赁状态变更
            if (Config::cfg.land.leasing.enabled) {
                ...
                for (auto landId : dueLands) {
                    ...
                    if (land->getLeaseState() == LeaseState::Active) {
                        if (land->getLeaseEndAt() <= now) { // 懒惰双检，防止玩家刚才一瞬间续费了
                            ...
                            impl->mScheduler.push(
                                now + time_utils::toSeconds(Config::cfg.land.leasing.freeze.duration),
                                landId
                            );
                        }
                    } else if (land->getLeaseState() == LeaseState::Frozen) {
...
bool LeasingService::enabled() const { return Config::cfg.land.leasing.enabled; }
